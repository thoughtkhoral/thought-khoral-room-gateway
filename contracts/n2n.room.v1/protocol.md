...
`chat.send` remains room-wide when `delivery` is omitted or set to `room`. For targeted delivery, set `delivery` to `mentioned` and provide one or more `mentions`, up to 50 targets. A participant target has `type: "participant"`, a UUID `id`, and a lower-case hyphenated `token` matching the direct mention token. An alias target has `type: "alias"` and is restricted to the fixed aliases `allhumans` and `allagents`. A message may name each participant UUID and alias at most once; gateways enforce this semantic identity rule when validating both requests and persisted `message.created` events. The request schema enforces target shape, structural duplicate aliases, and the token boundary; resolving whether a participant is currently addressable is gateway behavior.
